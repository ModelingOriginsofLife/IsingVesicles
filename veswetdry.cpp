...
- Now add bitvector for chemical species that percipitate pairise
************/
...
#define NCHEM 30
float Ocean[NCHEM];
float Waterchem[NCHEM];
float Oilchem[NCHEM];
float Bdychem[NCHEM];
float Ewater[NCHEM];            // energetic coupling to Hamiltonian
float Ebdy[NCHEM];              // used in localEnergy() 
float Eoil[NCHEM];              // -1 < Ecouple < 1
...
public:
    ...
    float mx,my,bmx,bmy;
    float temp;
    float chems[NCHEM];     // will be concentrations, 0 < ci < 1
    float wdcouple;             // strength of coupling to chemicals
    void Backup();
    void Restore();
    void Chemupdate();
}; 
...
void Cell::Chemupdate()
{
    int i;
    switch(phase)               // 0 oil, 1 bdy, 2 water
    {
    case 0: 
        for(i=0;i<NCHEM;i++)
            chems[i] = Oilchem[i];
...
float localEnergy(int x, int y)
{
    ...
    float wdcouple;
    float *Edel;
    chems = Grid[x+y*XR].chems; // ptr to chems array...
    wdcouple = Grid[x+y*XR].wdcouple; // strength of coupling
    switch(local)                     // local phase
    {
    case 0:                     // oil
        Edel = Eoil;
    case 1:
...
void Iterate()
{
    ...
    Grid[x+y*XR].Chemupdate();  // update to current phase.
...
void Init()
{
    ...
    for(i=0;i<NCHEM;i++){
        Ocean[i] = (rand()%10000000) / 10000000.0; // rnd in [0,1]
        Waterchem[i] = (rand()%10000000) / 10000000.0; // rnd in [0,1]
        Oilchem[i] = (rand()%10000000) / 10000000.0; // rnd in [0,1]
        Bdychem[i] = (rand()%10000000) / 10000000.0; // rnd in [0,1]
        Ewater[i] = (rand()%20000000) / 10000000.0 -1.0; // rnd in [-1,1]
        Ebdy[i] = (rand()%20000000) / 10000000.0 -1.0; // rnd in [-1,1]
        Eoil[i] = (rand()%20000000) / 10000000.0 -1.0; // rnd in [-1,1]
    }
	...
	for (y=0;y<YR;y++)
		for (x=0;x<XR;x++)
		{
            ...
            Grid[x+y*XR].wdcouple=(WDCOUPLE/2) + y*WDCOUPLE/(float)(YR);
        }            
	for (y=0;y<YR;y++)
		for (x=0;x<XR;x++)
		{
